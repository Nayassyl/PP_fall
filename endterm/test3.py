...
m = Matrix(3, 4, [1,2,-3,-3,2,-5,4,13,5,4,-1,5])
print(m.gaussian_elimination())
...
#  def __add__(self, other): 
#   if self.rows != other.rows or self.cols != other.cols: 
#    raise ValueError("Not possible to add.") 
#   result = [[self.data[i][j] + other.data[i][j] for j in range(self.cols)] for i in range(self.rows)] 
#   return matrix(result) 
  
#  def __sub__(self, other): 
#   if self.rows != other.rows or self.cols != other.cols: 
#    raise ValueError("Not possible to subtract.") 
...
#  def __mul__(self, other): 
#   if self.cols != other.rows: 
#    raise ValueError("Not possible to multiply.") 
   
#   result = [[sum(self.data[i][k] * other.data[k][j] for k in range(other.rows)) for j in range(other.cols)] for i in range(self.rows)] 
#   return matrix(result) 
...
#  def determinant(self): 
#   if self.rows != self.cols: 
#    raise ValueError("Not possible to calculate determinant.") 
#   if self.rows == 1: 
#    return self.data[0][0] 
#   elif self.rows == 2: 
#    return self.data[0][0] * self.data[1][1] - self.data[0][1] * self.data[1][0] 
...
#    det = 0 
#    for col in range(self.cols): 
#     cofactor = (-1) ** col * self.data[0][col] * self.minor(0, col).determinant() 
#     det += cofactor 
#    return det 
...
#  @staticmethod 
#  def gram_schmidt(matrix_obj): 
#   vectors = matrix_obj.transpose().data 
#   for vector in vectors: 
#    if all(coord == 0 for coord in vector): 
#     raise ValueError("Vectors must be non-zero.") 
#   result = [vectors[0]] 
#   for i in range(1, len(vectors)): 
#    orthogonalized = vectors[i].copy() 
#    for j in range(i): 
#     projection = sum(vectors[i][k] * result[j][k] for k in range(len(vectors[i])))/sum(result[j][k] * result[j][k] for k in range(len(vectors[i]))) 
...
#   normalized_result = [[coord / math.sqrt(sum(c**2 for c in vector)) for coord in vector] for vector in result] 
#   return matrix(normalized_result).transpose() 
  
#  @staticmethod 
#  def qr_decomposition(matrix_obj): 
#   q = matrix.gram_schmidt(matrix_obj) 
#   r = q.transpose()*matrix_obj 
#   return q, r 
         
#  @staticmethod 
#  def qr_iteration(matrix_obj): 
#   iteration_limit = 1000 
#   for i in range(iteration_limit): 
#    q, r = matrix.qr_decomposition(matrix_obj) 
#    matrix_obj = r * q 
#   eigenvalues = [(matrix_obj.data[i][i]) for i in range(matrix_obj.rows)] 
#   return matrix_obj
  
     
   
  
# matrix_data1 = [[1, 0, 2], [0, 2, 0], [0, -1, 1]] 
# matrix1 = matrix(matrix_data1) 
# eigenvalues = matrix1.qr_iteration(matrix1) 
# q, r = matrix.qr_decomposition(matrix1)
# print(q)
# print(r)